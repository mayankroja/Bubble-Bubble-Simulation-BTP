...
void incFSI::AMGPressurePoisson()
{
...
void incFSI::ComputeFinalVelocityField(int lev)
{
	...
	amrex::Array4<int const> const &cfmask = cfmask_xvel_.Mask().const_array(mfi);
	    ...
	    {
	        u(i, j, k) = 0.0;//set u = 0 inside bubble and in coarse mesh 
	    }
        });
    }
	...
	amrex::Array4<int const> const &cfmask = cfmask_yvel_.Mask().const_array(mfi);
        ...
        [&] AMREX_GPU_DEVICE(int i, int j, int k) noexcept 
        {
            ...
            {
                v(i, j, k) = 0.0;//set u = 0 inside bubble and in coarse mesh 
		//amrex::Print()<<"at i j k = "<<i<<" "<<j<<'\n';
            }
